...
inlining function calls as the arguments to other functions. Avoiding these
styles makes it easy to throw in a console.log, add a breakpoint between two
function calls and modify existing code to add or remove intermediate steps as
part of new requirements.
...
find similar ones in any project you work in regularly (and please keep in mind,
it's an illustration of the principle - writing fake code is hard!).

Another obvious example where this can manifest is Array method chains (and
broadly chainable method calls). Of the two examples below, which would be
easier to debug or modify? If you say the 2nd one, well, I'm at least thankful
...
were an issue with the end result of the array, it would be due to one of the
conditions being off. Having three separate filter calls allows us to easily go
in and inspect which predicates(s) are wrong through any technique at any
intermediate step.

Both are easy to debug, but the second would require some rewriting to specific
conditions or add in new ones. There is a middle ground of course, though it
still fails to allow us to inspect the value of the entire array between each
predicate.
...
So which one do we use? The middle ground option or the clean single loop. This
is where the rubber meets the road so to speak and where you have to discern and
weigh the options. Does the array have under 1,000 values? If so I'd probably
opt for the 3 filter calls, otherwise I may select the more optimized approach.

It doesn't matter which you choose (to a degree, measure performance and adapt
code as needed), so long as your choice is debuggable.
